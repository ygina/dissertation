...
% The next two sections describe Robin, our design for a Sidekick protocol built
% around quACKs. This includes the setup and configuration of a Robin sidekick
...
% Sidekick protocol (\Cref{fig:payloads}), starting with the handshake.

% Sidekicks are distinguished from other types of proxies such as VPNs and
% transparent PEPs in that the proxy is on the path and the client is knowingly
% receiving in-network retransmissions. In order to discover Sidekick proxies on the
...
% announcing that they support Sidekick and their supported configurations, and the
