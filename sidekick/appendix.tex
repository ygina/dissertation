...
(\Cref{tab:sidekick:experimental-scenarios}) with $0\%$ and $1\%$ loss on the near
path segmenet.
...
(\Cref{sec:sidekick:sender}), including how they were derived and why the PACUBIC
...
becomes the bottleneck as loss rates increase (\Cref{fig:sidekick:fairness-line}).
...
high loss rates (\Cref{fig:sidekick:fairness-line}). Also, it doesn't consider loss on the
