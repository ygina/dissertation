...
number to the proxy on the wire, but then that field risks ossification in the
future. This technical problem is the crux to enabling the transport behavior
...
computation costs (\Cref{sec:quack:psum-microbenchmarks:bit-widths}).
...
from the sender (every other packet by default in the Linux kernel).
...
the quACK, and the algorithmic complexities of their costs. \gina{Fill in this
summary.} Next we will explore efficient implementations of the
set-reconciliation designs and their computation and communication costs
compared to the strawmen in practice.

\section{Implementation}
\label{sec:quack:implementation}

The implementation of the quACK must be practical for packet-scale settings
...
Both constructions can practically decode set differences at
millisecond-RTT timescales and encode packets with nanosecond interarrival
times, summarized in \Cref{tab:quack:practical}.
...
algorithm needed to be fast so that the computational complexity outweighed
the constant factor overheads even when $t$ was small.
...
\input{quack/figures_tex/psum.tex}
...
identifier and settle on using 32-bit packet identifiers as the best tradeoff
between collision probability, communication size, and computation cost. We
analyze optimizations such as the decoding method for the power sum quACK and
show that our implementation can achieve practical packet encoding and quACK
decoding times in packet-scale settings. Our microbenchmarks used an m4.xlarge
AWS instance with a 4-CPU Intel Xeon E5 processor @ 2.30 GHz and 16 GB memory.
...
\Cref{tab:quack:optimized-quack} shows the results of our ablation experiments
for optimizations at different bit widths. In the remainder
...
(\Cref{tab:quack:practical}). These constructions are efficient to
