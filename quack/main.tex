...
\input{quack/tables/strawmen-practical.tex}
...
When $n=25$, \gina{use $n=500$?} using $4$ bytes results in an almost negligible
chance of collision while using $2$ bytes results in a 0.04\% chance
...
certain $m$. Using the power sum quACK, this is exactly $m$ symbols. In the IBLT
quACK, $4 \cdot m$ symbols have at least a $\!98.8\%$ success rate for all $m$
evaluated in \Cref{fig:quack:iblt-correctness}. When $m$ is large, the IBLT
quACK utilizes more of the link, but maintaining more symbols at the sender and
...
\input{quack/tables/strawmen-theoretical.tex}

The power sum and IBLT constructions of the quACK are efficient to decode, add
reasonable link overhead, and are cumulative representations of the packets seen
by the receiver (\Cref{tab:quack:strawmen-theoretical}). Compared to Strawman 2,
