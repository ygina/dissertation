...
(\Cref{sec:design}) and its implementation in two base protocols (\Cref
{sec:implementation}), and then evaluates the protocol in real-world and
emulation experiments (\Cref{sec:evaluation}).
...
split TCP connection (\Cref{sec:design:cubic}). The application considers
...
\Cref{fig:loss-vs-tput} shows that QUIC with a sidekick roughly matches---as
 ...
 its path-aware congestion-control scheme (\Cref{sec:design:cubic}). The
