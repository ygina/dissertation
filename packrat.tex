...
\begin{itemize}[noitemsep]
    ...
    \item An emulation study of the performance enhancements, as well as the CPU,
     memory, and link overheads of using the Packrat protocol (\Cref{sec:packrat:emulation}).
\end{itemize}
...
\section{Sending quACKs at the data receiver}
\label{sec:packrat:receiver}
...
\subsection{Selective quACKs}
\label{sec:packrat:receiver:selective}
...
(\Cref{sec:packrat:receiver}). We also
incorporate a reorder delay in the base connection (\Cref{sec:packrat:problem}).
...
each packet as described in \Cref{sec:packrat:proxy}. We use
...
\begin{enumerate}[noitemsep]
    ...
    \item How do the rateless and selective optimizations in \Cref
     {sec:packrat:receiver:rateless} impact the link overheads of the Packrat protocol?
...
it receives a quACK (\Cref{fig:memory:http}). The minimum cache size is
...
since the client selectively quACKs (\Cref{fig:memory:media}). The
...
the memory overheads of the per-client virtual caches (\Cref{fig:memory:multicast}).
